...
def continuous_dixon_coles_nll(params, df, n_teams, static_gamma, decay_rate=0.0018):
    """
    ...
    Gamma is now passed as a static constant to prevent optimization drift.
    """
    # 1. Unpack parameters
    alphas = params[0:n_teams]
    betas = params[n_teams:2*n_teams]
    rho = params[2*n_teams] # Low-score joint probability correction
    ...
    # Applying the static macro-gamma to home lambdas
...
def train_dixon_coles_prior(df_ledger, teams_list):
    ...
    Extracts a static gamma macro-ratio prior to optimization.
    """
    n_teams = len(teams_list)
    
    # Pre-compute static macro-gamma to prevent optimization drift
    total_home_xg = df_ledger['obs_xg_h'].sum()
    total_away_xg = df_ledger['obs_xg_a'].sum()
    static_gamma = total_home_xg / max(total_away_xg, 1.0)
    
    # Initial Parameter Guesses: Alphas=1.0, Betas=1.0, Rho=0.0
    init_params = np.concatenate([np.ones(n_teams), np.ones(n_teams), [0.0]])
    ...
    bounds = alpha_bounds + beta_bounds + rho_bounds
    ...
    print(f"Initiating Maximum Likelihood Estimation via SLSQP (Static Gamma = {static_gamma:.3f})...")
    res = minimize(
        fun=continuous_dixon_coles_nll,
        x0=init_params,
        args=(df_ledger, n_teams, static_gamma),
        method='SLSQP',
        bounds=bounds,
    ...
    meta_params = {
        ...
        'rho_draw_correction': float(optimized_params[2 * n_teams])
    }
